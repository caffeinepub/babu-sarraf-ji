...
// Apply the data migration function from the migration module

actor {
  // Capture the user permissions state automatically
  ...
  let backgrounds = Map.empty<Principal, Background>();
  ...
  public shared ({ caller }) func saveBackground(background : Storage.ExternalBlob) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("You must sign in to upload a background. Please sign in and try again.");
    };
    let newBackground : Background = {
      user = caller;
      background;
    };
    backgrounds.add(caller, newBackground);
  };

  public query ({ caller }) func getBackground() : async ?Storage.ExternalBlob {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("You must be signed in to fetch your background. Please sign in and try again.");
    };
    switch (backgrounds.get(caller)) {
      case (null) { Runtime.trap("No background set") };
      case (?background) { ?background.background };
    };
  };

  public shared ({ caller }) func removeBackground() : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("You must be signed in to remove your background. Please sign in and try again.");
    };
    backgrounds.remove(caller);
  };
